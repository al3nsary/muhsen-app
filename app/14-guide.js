...
function screenGuide() {
  ...
  return bar('دليل المهام', { back: 1, right: canEditGuide()
    ...
    '<div class="lbl">الشرح المتحرك<small>مقطع قصير يلخّص التنفيذ</small></div>' +
    guideClip(kind) +
...
/* ---------- الشرح المتحرك: مقطع مولَّد داخل التطبيق لكل نوع مهمة ---------- */
const CLIP_SCENES = 5, CLIP_SEC = 3;      /* ٥ مشاهد × ٣ ثوانٍ = ١٥ ثانية */

const CLIPS = {
  airport: [
    ['i-list',  'قبل الوصول',   'اطبع كشف الوصول وطابق العدد برقم الرحلة'],
    ['i-target','قبل ساعتين',   'كن في صالة الحج وأثبت حضورك من داخل النطاق'],
    ['i-users', 'الاستقبال',    'ارفع لوحة المجموعة ووزّع الأساور قبل الخروج'],
    ['i-check', 'العدّ مرتين',  'عند التجميع، ثم عند اكتمال صعود الحافلة'],
    ['i-send',  'التسليم',      'سلّم الجوازات بكشف موقّع ثم انطلق']
  ],
  checkin: [
    ['i-key',   'جاهزية السكن', 'افحص عينة من الغرف قبل وصول الحجاج بساعتين'],
    ['i-check', 'المفاتيح',     'استلمها بكشف مرقّم ووثّق الاستلام بصورة'],
    ['i-users', 'الأولوية',     'ابدأ بكبار السن وذوي الحالات الصحية'],
    ['i-ticket','بطاقات نُسك',  'سلّمها يدًا بيد مع التحقق من الاسم'],
    ['i-shield','قبل الإغلاق',  'مُرّ على الأدوار وسجّل أي ملاحظة كتذكرة']
  ],
  umrah: [
    ['i-check', 'قبل الخروج',   'تحقق من البطاقات والإحرام ونقطة التجمّع'],
    ['i-pin',   'نقطة ثابتة',   'حدّد نقطة تجمّع واحدة وأعلنها للجميع'],
    ['i-users', 'موقعك',        'ابقَ في مؤخرة المجموعة لا في مقدمتها'],
    ['i-info',  'الإرشاد',      'اشرح المناسك قبل الدخول لا أثناءه'],
    ['i-checkc','العودة',       'عُدّ المجموعة عند الباب قبل التحرّك']
  ],
  tour: [
    ['i-bus',   'التنسيق',      'أكّد الحافلة والسائق قبل الموعد بيوم'],
    ['i-list',  'المقاعد',      'وزّع الحجاج بكشف وتحقق من العدد'],
    ['i-clock', 'وقت العودة',   'أعلنه بوضوح عند الوصول للمزار'],
    ['i-camera','التوثيق',      'صوّر المجموعة — الصورة إثبات لا ذكرى'],
    ['i-checkc','قبل التحرّك',  'عدّ نهائي ثم انطلق']
  ],
  mina: [
    ['i-tent',  'جاهزية المخيم','افحص الفرش والمياه والتكييف قبل الفوج'],
    ['i-bus',   'التفويج',      'التزم الجدول ولا تُقلع قبل اكتمال العدد'],
    ['i-list',  'التوزيع',      'وزّع الأماكن بكشف مكتوب'],
    ['i-users', 'الأولوية',     'كبار السن قرب المخارج ودورات المياه'],
    ['i-check', 'الضيافة',      'تحقق من وصول الوجبة في وقتها']
  ],
  arafah: [
    ['i-sun',   'الوصول',       'سجّل العدد فور الاستقرار في المخيم'],
    ['i-med',   'الحالات',      'راقب الإجهاد الحراري — أخطر ما في اليوم'],
    ['i-check', 'الماء',        'وزّع المياه بانتظام طوال النهار'],
    ['i-clock', 'قبل الغروب',   'جهّز المجموعة للمغادرة إلى مزدلفة'],
    ['i-send',  'الدفع',        'أعلن نقطة التجمّع مبكرًا وانطلق']
  ],
  jamarat: [
    ['i-clock', 'الجدول',       'التزم بموعد الرمي المعتمد مهما كان الضغط'],
    ['i-users', 'المرافقة',     'رافق المجموعة ذهابًا وإيابًا'],
    ['i-pin',   'نقطة التجمّع', 'حدّدها بعد الرمي وأعلنها قبل الدخول'],
    ['i-target','الرمي',        'تابع التنفيذ ولا تترك أحدًا وحده'],
    ['i-checkc','بعد العودة',   'عُدّ الحجاج فور الوصول للمخيم']
  ],
  wada: [
    ['i-list',  'التجهيز',      'أنهِ الحقائب قبل الطواف'],
    ['i-ticket','البطاقات',     'تأكد أن الجميع يحمل بطاقة نُسك'],
    ['i-kaaba', 'الطواف',       'ذكّر بأن الوداع آخر العهد بالبيت'],
    ['i-pin',   'باب واحد',     'نظّم الخروج من باب واحد فقط'],
    ['i-checkc','العدّ',        'عُدّ المجموعة قبل مغادرة الحرم']
  ],
  checkout: [
    ['i-list',  'قبل يوم',      'جهّز كشف المغادرة واختم قائمة الركاب'],
    ['i-clock', 'قبل ساعتين',   'ابدأ التجميع وتحقق من عدد الحقائب'],
    ['i-send',  'الجوازات',     'سلّمها للسائق بكشف واحتفظ بنسخة'],
    ['i-bus',   'الصعود',       'تحقق من العدد على الحافلة'],
    ['i-checkc','التأكيد',      'لا تغادر قبل تأكيد اكتمال العدد']
  ]
};
const clipOf = k => CLIPS[k] || CLIPS.airport;

function guideClip(kind) {
  const ui = KIND_UI[kind] || { i:'i-tasks', c:'#5A6C63' };
  const sc = clipOf(kind);
  const total = CLIP_SCENES * CLIP_SEC;
  const paused = S.clipPaused ? ' paused' : '';
  return '<div class="clip' + paused + '" style="--kc:' + ui.c + ';--dur:' + total + 's">' +
    '<div class="stage">' +
      sc.map((s, i) => '<div class="scene" style="animation-delay:' + (i * CLIP_SEC) + 's">' +
        '<span class="sic">' + icon(s[0], 's26') + '</span>' +
        '<b>' + E(s[1]) + '</b><span>' + E(s[2]) + '</span></div>').join('') +
      '<span class="badge2">' + icon(ui.i, 's14') + E(CAT[kind].ar) + '</span>' +
    '</div>' +
    '<div class="ctrl">' +
      '<button class="pb" data-a="clipplay" aria-label="' + (S.clipPaused ? 'تشغيل' : 'إيقاف') + '">' +
        icon(S.clipPaused ? 'i-play' : 'i-stop', 's16') + '</button>' +
      '<span class="bar"><i></i></span>' +
      '<span class="tm">' + AR(total) + ' ثانية</span>' +
    '</div></div>';
}
